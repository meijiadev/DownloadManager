...
public class NProxyCacheManager implements ICacheManager, CacheListener {
    ...
    private static NProxyCacheManager proxyCacheManager;
     ...
     * 单例管理器
     */
    public static synchronized NProxyCacheManager instance() {
        if (proxyCacheManager == null) {
            proxyCacheManager = new NProxyCacheManager();
        }
    ...
    public void clearCache(Context context, File cachePath, String url) {
        if (TextUtils.isEmpty(url)) {
            ...
            if (NProxyCacheManager.fileNameGenerator != null) {
                md5FileNameGenerator = NProxyCacheManager.fileNameGenerator;
            }
    ...
    protected static HttpProxyCacheServer getProxy(Context context) {
        HttpProxyCacheServer proxy = NProxyCacheManager.instance().proxy;
        return proxy == null ? (NProxyCacheManager.instance().proxy =
                NProxyCacheManager.instance().newProxy(context)) : proxy;
    }
    ...
    public static HttpProxyCacheServer getProxy(Context context, File file) {
        ...
        if (NProxyCacheManager.instance().mCacheDir != null
                && !NProxyCacheManager.instance().mCacheDir.getAbsolutePath().equals(file.getAbsolutePath())) {
            //不一致先关了旧的
            HttpProxyCacheServer proxy = NProxyCacheManager.instance().proxy;
            ...
            //开启新的
            return (NProxyCacheManager.instance().proxy =
                    NProxyCacheManager.instance().newProxy(context, file));
        } else {
            //还没有缓存文件的或者一致的，返回原来
            HttpProxyCacheServer proxy = NProxyCacheManager.instance().proxy;

            return proxy == null ? (NProxyCacheManager.instance().proxy =
                    NProxyCacheManager.instance().newProxy(context, file)) : proxy;
        }
    }

    public static void setFileNameGenerator(FileNameGenerator fileNameGenerator) {
        NProxyCacheManager.fileNameGenerator = fileNameGenerator;
    }

    public static void clearFileNameGenerator() {
        NProxyCacheManager.fileNameGenerator = null;
    }
}
